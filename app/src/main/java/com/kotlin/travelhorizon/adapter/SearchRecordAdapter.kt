...
class SearchRecordAdapter(private val recordList: MutableList<Dto>): RecyclerView.Adapter<SearchRecordAdapter.ListItemHolder>()
{
    ...
    fun updateList(newList: List<Dto>) {
        recordList.clear()
    ...
    inner class ListItemHolder(private val binding: ListItemBinding) : RecyclerView.ViewHolder(binding.root) {
        ...
        init {
            binding.recordListItem.setOnClickListener(View.OnClickListener {
                ...
                findNavController(it).navigate(R.id.action_searchFragment_to_viewFragment, bundle)
            })
